...
*/
	...
	function colPageList($url,$classname=''){
		$url=$this->handleUrl($url);
		if($this->startPoint[1]!=false){
			$pid=$this->checkUrl($url,1,true,$classname);
			if(!$pid){
				return false;
			}
		}
		...
		if($listNext!=false){
			foreach($listNext as $k=>$v){	
					$this->colPageList($this->site.$v['href']);
			}
		}
	}
	function onePage($url,$grade=2){var_dump($url);
		$url=$this->handleUrl($url);
		$pid=$this->checkUrl($url,$grade);var_dump($pid);
		if(!$pid){ return ;}
		$contents=false;
	...
	/*
		...
		2代表为单条数据
	*/
	function checkUrl($url,$grade=1,$startBool=false,$classname=''){
		if(empty($url)){return ;}
